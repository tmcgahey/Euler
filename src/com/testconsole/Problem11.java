...
public class Problem11 implements ProblemSolver {
    ...
    public String solveProblem() {
        ...
        int rowCount = 0;
        int columnCount = 0;
        for(String num : numberStringArray )
        {
            numberGrid[rowCount][columnCount] = Integer.parseInt(num);
            columnCount++;
        ...
        for(int i = 0; i < 16; i++)
        {
            for(int j = 0; j < 16; j++)
            {
                ...
                //diagonal
                int leftToRight = numberGrid[i][j] * numberGrid[i+1][j+1] * numberGrid[i+2][j+2] * numberGrid[i+3][j+3];
                if(leftToRight > greatestProduct)
                {
                    greatestProduct = leftToRight;
                }

                int rightToLeft = numberGrid[i][j+3] * numberGrid[i+1][j+2] * numberGrid[i+2][j+1] * numberGrid[i+3][j];
                if(rightToLeft > greatestProduct)
                {
                    greatestProduct = rightToLeft;
                }
            }
        }
